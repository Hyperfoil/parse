...
- link:./asContent.adoc[asContent] - load the entire file content as a string
- asJbossCli - parse the jboss-cli output into json
- asJson - assume the file contains json
- link:./asText.adoc[asText] - parse each line of text
- link:./asPath.adoc[asPath] - execute a javascript function on the file path
