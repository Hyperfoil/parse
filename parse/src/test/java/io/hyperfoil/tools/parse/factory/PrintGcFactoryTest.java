...
public class PrintGcFactoryTest {
    ...
    public void newParser_cms_g1TagBug(){
        ...
        Assert.assertFalse("root.g1Tag",root.has("g1Tag"));
    }
    ...
    public void newParser_cms_ParNew_tenuringDistribution(){
        ...
        Assert.assertTrue("root.times",root.has("times") && root.get("times") instanceof Json);

        Assert.assertEquals("root.reason","Allocation Failure",root.getString("reason"));
        Assert.assertEquals("root.datestamp","2018-05-10T04:51:23.342+0000",root.getString("datestamp"));
        Assert.assertEquals("root.timestamp",1.085,root.getDouble("timestamp"),0.00000001);
        Assert.assertEquals("root.gcId",0,root.getLong("gcId"));
        Assert.assertEquals("root.before\n"+root.toString(2), StringUtil.parseKMG("6710912K"),root.getLong("before"));
        Assert.assertEquals("root.after", StringUtil.parseKMG("13897K"),root.getLong("after"));
        Assert.assertEquals("root.capacity", StringUtil.parseKMG("32715584K"),root.getLong("capacity"));
        Assert.assertTrue("root has region",root.has("region") && root.get("region") instanceof Json);
        Json region = root.getJson("region");
        Assert.assertEquals("region.size",1,region.size());

        Json region0 = region.getJson(0);
        Assert.assertEquals("region[0].region","ParNew",region0.getString("region"));
        Assert.assertEquals("region[0].before", StringUtil.parseKMG("6710912K"),region0.getLong("before"));
        Assert.assertEquals("region[0].after", StringUtil.parseKMG("13897K"),region0.getLong("after"));
        Assert.assertEquals("region[0].capacity", StringUtil.parseKMG("7549760K"),region0.getLong("capacity"));

        Assert.assertTrue("root.tenuring",root.has("tenuring") && root.get("tenuring") instanceof Json);
    ...
    public void newParser_cms_promotionFailed(){
        ...
        Assert.assertTrue("has region",root.has("region") && root.get("region") instanceof Json);

        Json region = root.getJson("region");
        Assert.assertEquals("3 regions:\n"+region.toString(2),3,region.size());

        Set<String> names = region.values().stream().map(s->((Json)s).getString("region")).collect(Collectors.toSet());
        Assert.assertTrue("names: "+names,names.containsAll(Sets.of("ParNew","CMS","Metaspace")));
        ...
        Json parNew = region.values().stream()
            ...
            .findFirst()
            .orElse(new Json());
        Assert.assertEquals("parNew warning","promotion failed",parNew.getString("warning"));
    }
...
//        p.onLine("    S: JVMTI Roots          =     0.00 s (a =        2 us) (n =     6) (lvls, us =        1,        1,        1,        1,        3)");
    ...
    public void newParser_serial_gc_prefixed(){
        ...
        Assert.assertTrue("expected keys missing:"+Sets.unique(expectedKeys,root.keys())+":\n"+root.toString(2),
            root.keys().containsAll(expectedKeys)
        );
        Assert.assertEquals("before 180052K",180052*1024,root.getLong("before"));
        Assert.assertEquals("after 50260K",50260*1024,root.getLong("after"));
        Assert.assertEquals("capacity 287424K",287424*1024,root.getLong("capacity"));
    ...
    public void newParser_serial_gcDetails_prefixed(){
        ...
        Assert.assertTrue("expected keys:"+Sets.unique(expectedKeys,root.keys())+":\n"+root.toString(2),
                root.keys().containsAll(expectedKeys))
        ;

        Assert.assertEquals("root before 181403K",181403*1024,root.getLong("before"));
        Assert.assertEquals("root after 50260K",50260*1024,root.getLong("after"));
        Assert.assertEquals("root capacity 287424K",287424*1024,root.getLong("capacity"));
        Assert.assertEquals("root seconds 0.0287714",0.0287714,root.getDouble("seconds"),0.0000001);
    }
    ...
    public void newParser_serial_gcDetails_heap(){
        ...
        Assert.assertTrue("has keys "+root.keys(),root.keys().containsAll(Sets.of(
                "tenured generation","def new generation","Metaspace"
        )));
        Assert.assertEquals("3 keys "+root.keys(),3,root.keys().size());
    ...
    public void newParser_serial_heapAtGc_before(){
        ...
        Assert.assertTrue("has keys "+root.toString(2),root.keys().containsAll(Sets.of(
                "tenured generation","def new generation","Metaspace", "phase", "gcCount","fullCount"
        )));
        Assert.assertEquals("6 keys "+root.keys(),6,root.keys().size());
    ...
    public void newParser_parallel_gcDetails_fullGc(){
        ...
        Assert.assertTrue("missing keys:"+Sets.unique(expectedKeys,root.keys())+":\n"+root.toString(2),root.keys().containsAll(expectedKeys));
    ...
    public void newParser_g1gc_prefix(){
        ...
        Assert.assertTrue("expect keys missing "+Sets.unique(expectedKeys,root.keys())+":\n"+root.toString(2),root.keys().containsAll(expectedKeys));

        Assert.assertEquals("reason","G1 Humongous Allocation",root.getString("reason"));
        Assert.assertEquals("seconds",0.003346,root.getDouble("seconds"),0.0000001);
        Assert.assertEquals("before",98*1024*1024,root.getLong("before"));
        Assert.assertEquals("after",19*1024*1024,root.getLong("after"));
        Assert.assertEquals("capacity",250*1024*1024,root.getLong("capacity"));
        Assert.assertTrue("tags has 2 entires",root.get("g1Tag") instanceof Json && root.getJson("g1Tag").size()==2);
    ...
    public void newParser_g1gc_phase_remark(){
        ...
        Assert.assertEquals("phase","remark",root.getString("phase"));
        Assert.assertEquals("seconds",0.0091266,root.getDouble("seconds"),0.00000001);
    }
    ...
    public void newParser_g1gc_phase_cleanup(){
        ...
        Assert.assertEquals("phase","cleanup",root.getString("phase"));
        Assert.assertEquals("before",39*1024*1024,root.getLong("before"));
        Assert.assertEquals("after",39*1024*1024,root.getLong("after"));
        Assert.assertEquals("capacity",250*1024*1024,root.getLong("capacity"));
        Assert.assertEquals("seconds",0.0004707,root.getDouble("seconds"),0.00000001);
    ...
    public void newParser_g1gc_details_nest(){
        ...
        Assert.assertEquals("phase","pause",root.getString("phase"));
        Assert.assertEquals("reason","G1 Humongous Allocation",root.getString("reason"));
        Assert.assertEquals("seconds",0.0026001,root.getDouble("seconds"),0.00000001);
        Assert.assertTrue("g1Tag",root.has("g1Tag") && root.getJson("g1Tag").isArray());
        Assert.assertTrue("g1Tag=[young,initial-mark]",root.getJson("g1Tag").values().containsAll(Arrays.asList("young","initial-mark")));
        Assert.assertTrue("detail",root.has("detail") && root.getJson("detail").isArray());
        Assert.assertEquals("detail count:\n"+root.getJson("detail").values().stream().map(v->{
        ...
        Assert.assertEquals("detail[0].millliseconds\n"+detail0.toString(2),1.6,detail0.getDouble("millliseconds"),0.00000001);
        Assert.assertEquals("detail[0].worker",4,detail0.getLong("workers"));
        Assert.assertTrue("detail[0].detail",detail0.has("detail") && detail0.getJson("detail").isArray());

        Json detail00 = detail0.getJson("detail").getJson(0);
        Assert.assertEquals("detail[0][0].category","GC Worker Start (ms)",detail00.getString("category"));
        Assert.assertEquals("detail[0][0].min",104.7,detail00.getDouble("min"),0.00000001);
        Assert.assertEquals("detail[0][0].avg",104.8,detail00.getDouble("avg"),0.00000001);
        Assert.assertEquals("detail[0][0].max",105,detail00.getDouble("max"),0.00000001);
        Assert.assertEquals("detail[0][0].diff",0.3,detail00.getDouble("diff"),0.00000001);

        Json detail1 = root.getJson("detail").getJson(1);

        Assert.assertEquals("detail[1].category","Code Root Fixup",detail1.getString("category"));
        Assert.assertEquals("detail[1].time",0,detail1.getDouble("time"),0.00000001);
    ...
    public void gcShenandoahDetailsInlineTotalGarbage(){
        ...
        Assert.assertFalse("root should not be empty",root.isEmpty());
    ...
    public void gcShenandoahDetailsHeapVirtualRange(){
        ...
        Assert.assertFalse("root should not be empty",root.isEmpty());
        Assert.assertEquals("root.low_b","0x0000000340000000",root.getString("low_b"));
        Assert.assertEquals("root.high_b","0x00000007c0000000",root.getString("high_b"));
    ...
    public void gcDateStamps_minus(){
        ...
        Assert.assertTrue("has datestamp:"+root.toString(),root.has("datestamp"));
        Assert.assertEquals("2018-04-17T10:42:28.747-0500",root.getString("datestamp"));
    }
    @Test
    ...
    public void gcTimeStamps(){
        ...
        Assert.assertTrue("has timestamp:"+root.toString(),root.has("timestamp"));
        Assert.assertEquals(0.076,root.getDouble("timestamp"),0.0001);
    }
    @Test
    public void gcId(){
        Json root =f.gcId().apply("2018-04-17T10:42:28.747-0500: 0.076: #0: [GC (Allocation Failure)  61852K->15323K(247488K), 0.0066592 secs]");
        Assert.assertTrue("has gcId:"+root.toString(),root.has("gcId"));
        Assert.assertEquals(0,root.getLong("gcId"));
    }
    @Test @Ignore
    ...
    public void gcType_gc(){
        ...
        Assert.assertTrue("has type:"+root.toString(),root.has("type"));
        Assert.assertEquals("GC",root.getString("type"));
    }
    @Test
    public void gcType_fullGc(){
        ...
        root = p.getBuilder().getRoot();
        Assert.assertTrue("has type:"+root.toString(),root.has("type"));
        Assert.assertEquals("Full GC",root.getString("type"));
    }
    @Test
    public void gcResize(){
        ...
        root = p.getBuilder().getRoot();
        Assert.assertTrue("has before:"+root.toString(),root.has("before"));
        Assert.assertTrue("has after:"+root.toString(),root.has("after"));
        Assert.assertTrue("has capacity:"+root.toString(),root.has("capacity"));
    }
    @Test
    public void gcSecs(){
        Parser p = new Parser();
        ...
        p.onLine("2018-04-17T10:42:28.747-0500: 0.076: #0: [GC (Allocation Failure)  61852K->15323K(247488K), 0.0066592 secs]");
        root = p.getBuilder().getRoot();
        Assert.assertTrue("has seconds:"+root.toString(),root.has("seconds"));
        Assert.assertEquals(0.0066592,root.getDouble("seconds"),0.0000001);
    }
    @Test
    public void gcCmsUsed(){
        ...
        root = p.getBuilder().getRoot();
        Assert.assertTrue("has used:"+root.toString(),root.has("used"));
        Assert.assertTrue("has capacity:"+root.toString(),root.has("capacity"));
    }
    @Test
    ...
    public void gcCmsTimed(){
        ...
        Assert.assertEquals("phase","Rescan (parallel)",root.getString("phase"));
        Assert.assertEquals("secs",0.0004003,root.getDouble("secs"),0.00000001);

        root = f.gcCmsTimed().apply("[weak refs processing, 0.0000055 secs]");
        Assert.assertEquals("phase","weak refs processing",root.getString("phase"));
        Assert.assertEquals("secs",0.0000055,root.getDouble("secs"),0.00000001);

        root = f.gcCmsTimed().apply("[class unloading, 0.0001464 secs]");
        Assert.assertEquals("phase","class unloading",root.getString("phase"));
        Assert.assertEquals("secs",0.0001464,root.getDouble("secs"),0.00000001);

        root = f.gcCmsTimed().apply("[scrub symbol table, 0.0001936 secs]");
        Assert.assertEquals("phase","scrub symbol table",root.getString("phase"));
        Assert.assertEquals("secs",0.0001936,root.getDouble("secs"),0.00000001);
    }
    ...
    public void gcDetailsRegionName(){
        ...
        Assert.assertEquals("region","1 CMS-initial-mark",root.getString("region"));

        root = f.gcDetailsRegionName().apply("[ParNew: 0K->0K(166720K), 0.0006483 secs]");
        Assert.assertEquals("region","ParNew",root.getString("region"));
    }
    @Test
    public void gcDetailsTimes(){
        ...
        Assert.assertTrue("has user,sys,real:"+root.toString(),
    ...
    public void gcHeapAtGcHeader_before(){
        ...
        Assert.assertTrue("has phase,gcCount,fullCount:"+root,
    ...
    public void gcDetailsHeapRegion(){
        ...
        Assert.assertEquals("region is def new generation","def new generation",root.getString("region"));
    }
    @Test
    public void gcDetailsHeapSpace(){
        ...
        Assert.assertEquals("space is eden","eden",root.getString("space"));
    }
    @Test
    public void gcDetailsHeapMeta(){
        ...
        Assert.assertEquals("region","Metaspace",root.getString("region"));
    }
    @Test
    public void gcDetailsHeapMetaSpace(){
        ...
        Assert.assertEquals("space is class","class",root.getString("space"));

    }
    @Test
    public void gcApplicationConcurrent(){
        Json root = f.gcApplicationConcurrent().apply(" Application time: 0.0435193 seconds");
        Assert.assertTrue("has 'applicationConcurrent':"+root,root.has("applicationConcurrent"));
    }
    @Test
    public void gcApplicationStopped(){
        ...
        Assert.assertTrue("has 'applicationStopped':"+root,root.has("applicationStopped"));
    }
