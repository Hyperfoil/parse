...
public class Exp2Test {
   ...
   public void child_beforeParent_eat_toMatch(){
      ...
      Exp b = new Exp("b","b=(?<b>\\w+)")
         .eat(Eat.ToMatch)
         .add(new Exp("a","a=(?<a>\\w+)").setTarget(MatchTarget.BeforeParent));
      JsonBuilder builder = new JsonBuilder();
   ...
   public void child_beforeParent_then_entireLine(){
      ...
      Exp b = new Exp("b","b=(?<b>\\w+)")
         .eat(Eat.ToMatch)
         .add(new Exp("a","a=(?<a>\\w+)").setTarget(MatchTarget.BeforeParent))
         .add(new Exp("again","a=(?<again>\\w+)").setTarget(MatchTarget.EntireLine));
...
//      assertEquals("should equal the offset from start of line", 7, b.getRoot().getLong("key"));
   ...
   public void lineStart(){
      JsonBuilder b = new JsonBuilder();
      Exp p = new Exp("kv", "(?<key>\\w+)=(?<value>\\w+)")
         .add( new Exp("-","(?<sk>\\w+)\\-(?<sv>\\w+)").setTarget(MatchTarget.EntireLine) );
