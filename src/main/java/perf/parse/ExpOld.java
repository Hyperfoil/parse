...
public class ExpOld {
    ...
    //matches (?<value> part of java named pattern matching
    ...
    private MatchTarget target = MatchTarget.AfterParent;
    ...
    public enum NestType {Name,Extend,Field}
    private class NestPair {
        private final String value;
        private final NestType type;
        public NestPair(String name, NestType type){
            this.value = name;
            this.type = type;
        }
        public String getValue(){return value;}
        public NestType getType(){return type;}
    }

    private LinkedList<NestPair> groupings;
    ...
    private LinkedList<ExpOld> children;
    ...
    public ExpOld(String name, String pattern){
        this.name = name;
    ...
    public ExpOld with(String key, Object value){
    ...
    public ExpOld clone(){
        ExpOld rtrn = new ExpOld(this.getName(),this.getPattern());
        ...
        for(NestPair pair : groupings){
            rtrn.groupings.add(new NestPair(pair.getValue(),pair.getType()));
        }
        for(ExpOld child : children){
    ...
    public ExpOld forEachField(Consumer<String> fieldVisitor){
        for(String value : fieldValues.keySet()){
    ...
    /**
     * Set the expression nesting using value:groupType.value:groupType...
     * @param nesting
     * @return
     */
    public ExpOld nest(String nesting){
        List<String> keys = chain(nesting);
        for(String key : keys){
            int index=-1;
            String name = key;
            String type = NestType.Name.toString();
            ...
            groupings.add(new NestPair(name, StringUtil.getEnum(type, NestType.class, NestType.Name)));
        }
        return this;
    }
    public ExpOld group(String name){
        groupings.add(new NestPair(name, NestType.Name));
        return this;
    }
    public ExpOld key(String name){
        groupings.add(new NestPair(name, NestType.Field));
        return this;
    }
    //
    public ExpOld extend(String name){
        groupings.add(new NestPair(name, NestType.Extend));
        return this;
    }

    /**
     * Set a custom width to to remove from the CheatChars input whenever the ExpOld matches
     * @param width the number of characters to remove
     * @return this
     */
    public ExpOld eat(int width){
        eat = width;
        return this;
    }
    /**
     * Defines how much of the input CheatChars should be removed if this ExpOld matches. The default is Eat.Match.
     ...
     * @return
     */
    public ExpOld execute(MatchAction action){
        callbacks.add(action);
        return this;
    }

    /**
     * Add the child ExpOld. child will run after each time the expression matches
     * @param child
     * @return
     */
    public ExpOld add(ExpOld child){
        children.add(child);
        return this;
    }
    public boolean hasChildren(){return !children.isEmpty();}
    public void eachChild(Consumer<ExpOld> consumer){
        children.forEach(consumer);
    }
    public int childCount(){return children.size();}
    public ExpOld set(String name, Value value){
        if(value == Value.Key){
            throw new IllegalArgumentException("set(String Name,Value value) cannot be used for Value.Field, use set(String Name,String valueKey)");
        }
        fieldValues.put(name,value.getId());
        return this;
    }
    public ExpOld set(String name, String valueKey){
    ...
    /**
     * Add a Rule for action to take when the perf.parse.ExpOld matches the input
     * @param rule the rule to add
     * @return this
     */
    public ExpOld set(Rule rule){
        rules.put(rule,rule);
        return this;
    }

    public ExpOld set(Rule rule, Object object){
        rules.put(rule,object);
        return this;
    }

    /**
     * Set how the matching Name value groups should be merged into the result of previous / subsequent ExpOld matches
     * @param merge
     * @return
     */
    public ExpOld set(Merge merge){
        this.merge = merge;
        return this;
    }
    public Merge getMerge(){return merge;}
    public int getEat(){return eat;}
    public void eachGroup(BiConsumer<String, NestType> consumer){
        groupings.forEach(p->consumer.accept(p.getValue(),p.getType()));
    }
    ...
    //if the ExpOld will eat the given amount
    public boolean is(Eat e){
        return Eat.from(this.eat).equals(e);
    }

    //if the ExpOld has the rule
    public boolean is(Rule r){
        return rules.containsKey(r);
    }

    //the number of times Rule was set on this ExpOld
    ...
    private boolean populate(IMatcher m,JsonBuilder builder){
        if(isDebug()){
            logger.info(getName()+" > populate ");
            logger.info("    "+builder.debug(true).replaceAll("\n","\n    "));
        }
    ...
    public boolean apply(DropString line, JsonBuilder builder, ParserOld parser){
        boolean result = apply(line,builder,parser,new AtomicInteger(0));
        return result;
    }

    public ExpOld enables(String state){
        this.enables.add(state);
        return this;
    }
    public ExpOld disables(String state){
        this.disables.add(state);
        return this;
    }
    public ExpOld requires(String state){
        this.requires.add(state);
        return this;
    }

    //changed to atomic integer so pattern can change start offset if it eats before start
    private boolean apply(DropString line, JsonBuilder builder, ParserOld parser, AtomicInteger previousEnd){

        //BUG some ExpOld match the empty line!!
        if(/*line.length()==0 ||*/ previousEnd.get() > line.length()){
            return false;
        }
        if(isDebug()){
            logger.info(this.getName()+" > apply @ "+previousEnd+": line=||"+line+"||");
            logger.info("  line@start=||"+line.subSequence(previousEnd.get(),line.length())+"||");
        }
        ...
        boolean rtrn = false;
        matcher.reset(line);
        int startPoint = is(Rule.LineStart) ? 0 : previousEnd.get();
        if(isDebug() && startPoint != previousEnd.get()){
        ...
        if(matcher.find()){
            ...
            if ( is(Merge.PreClose) ) {
                ...
                if(ruleInfo.isEmpty()) {//just pop an un-named target
                    if (isDebug()) {
                        System.out.println("    PrePopTarget");
                    }

                    builder.popTarget();
                }else{//pop all the named targets from the target stack
            ...
            do {
                ...
                for(Iterator<NestPair> groupIter = groupings.iterator(); groupIter.hasNext();){
                    //TODO handle case where groupName should be an integer key?
                    NestPair pair = groupIter.next();
                    String groupName = pair.getValue();
                    NestType groupType = pair.getType();
                    boolean extend = false;
                    if(isDebug()){
                        System.out.println("  grouping "+groupType+"="+groupName);
                    }
                    if( NestType.Field.equals(groupType) ){
                    ...
                    if( NestType.Extend.equals(groupType) ) {
                        extend=true;
                    }
                        ...
                        if ( is(Merge.Entry) ) {
                            ...
                            if( grouped.has(groupName) ) {
                                if( !(grouped.get(groupName) instanceof Json) ){
                                    System.out.println("groupName "+groupName+" not a group for ExpOld:"+getName());
                ...
                int currentStart = previousEnd.get();

                DropString childLine = line;
                AtomicInteger childStart = new AtomicInteger(mEnd);

                if(is(ChildrenLookBehind)){
                    childLine = (DropString)line.subSequence(0,mStart);
                    childStart.set(0);
                }
                switch(toEat){
                    case ToMatch:
                        ...
                        line.drop(0,mEnd);
                        if(previousEnd.get() > mEnd){
                            if(isDebug()){
                                System.out.println(getName()+"    set start = "+(previousEnd.get()-mEnd));
                            }
                            previousEnd.set(previousEnd.get()-mEnd);
                        }else{
                            if(isDebug()){
                    ...
                    case Width:
                        ...
                        if(currentStart <= mStart+wStop && currentStart >= mStart){
                            ...
                            previousEnd.set(currentStart- (wStop));
                        }
                        matcher.region(mStart,line.length());
                        mEnd = mStart;
                        break;
...
//                    System.out.println(this.getValue()+" changed start from "+currentStart+" to "+start.get()+" length="+line.length());
...
//                }
                ...
                do {
                    ...
                    for (ExpOld child : children) {
                        ...
                        boolean thisChildMatched = child.apply(childLine, builder, parser, childStart);
                        if(thisChildMatched){
                        ...
                        if(thisChildMatched && childStartBefore != childStart.get()){
                            ...
                            currentStart = previousEnd.get();
                            if( currentStart > childStart.get() && currentStart < childStartBefore){
                                previousEnd.set( childStart.get());
                            }else if( currentStart > childStart.get() ){
                                previousEnd.set( currentStart - (childStart.get() - childStartBefore));
                            }else{
                ...
                if( needPop && !changedContext) {
                    Json poped = builder.popTarget(getName()+GROUPED_NAME);//pop the group target for this ExpOld (preserves child targets)
                }
            ...
            if( is(Rule.Repeat) && rtrn){
                for (MatchAction action : callbacks) {
                    //action.onMatch(line.getLine(), target, this, parser);
                }
            }
